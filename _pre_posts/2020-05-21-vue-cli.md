...
自动解析确定的扩展。默认值为： `extensions: [".js", ".json"]`, 解析顺序为从左到右，如果目标文件后缀名为 `.ext`，同时希望 webpack 解析文件如代码 `import SomeFile from "./somefile"` 则需要在 extensions 配置加上 `.ext` 如： `extensions: [".js", ".json",".ext"]`。

#### modules

告诉 webpack 解析模块时应该搜索的目录，支持绝对路径和相对路径。
